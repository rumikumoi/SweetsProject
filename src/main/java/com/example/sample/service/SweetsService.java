...
public class SweetsService {
	...
	public SweetsForm getShopData() {
		...
		for (Sweets sweets : list) {
			...
			data.setUnitPrice(sweets.getUnitPrice());
			sweetsList.add(data);
		}
	 ...
	 */
	// 合計金額の計算
	public int calcTotal(List<SweetsData> list) {
		int total = 0;
		for (SweetsData sweets : list) {
			if (sweets.getPurchases() > 0) {
				total = total + (sweets.getUnitPrice() * sweets.getPurchases());
			}
		}
		return total;
	}
